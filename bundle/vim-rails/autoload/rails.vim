...
function! s:app_stylesheet_suffix() dict abort
  if self.cache.needs('stylesheet_suffix')
    let default = self.has_gem('sass-rails') ? '.css.scss' : '.css.scss'
    let candidates = map(filter(
...
function! s:BufCommands()
  ...
  command! -buffer -bar -nargs=? -bang Rroutes :call s:Routes()
...
"open routes file
function! s:Routes()
  let root = RailsRoot() . '/config/routes.rb'
  silent execute 'edit' l:root 
endfunction
